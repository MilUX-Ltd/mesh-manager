...
Start from the operator's standing brief: what this mesh is for, who runs it, its region and
channel policy, standing orders. Nothing in this file knows the name of a single radio, box,
channel or place, and that is deliberate: a role that hardcoded a fleet would be wrong on its
first line for everyone else.

The box offers the brief as a **resource**, `mesh://brief/context.md`, so a client that takes
resources may already have handed it to you. If you have it, do not fetch it again. If you do
not, call `mesh_context`, which is the same text through a tool and works in every client. The
box offers this role and the skills as resources too, at `mesh://brief/`, which is why you may
find you already have them.
