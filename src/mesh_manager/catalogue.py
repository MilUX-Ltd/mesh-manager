...
ACTIONS = [
    ...
    {"id": "gateway", "title": "The gateway radio", "risk": "read", "op": "gateway", "inputs": [],
     "description": "Which radio this box is using as its gateway, whether that radio is actually plugged in, whether the box is waiting for one to be chosen, and every radio plugged in that could be chosen instead. A GPS receiver the box is using for its position is not offered."},
    {"id": "gateway_export", "title": "Keep a copy of the gateway's configuration", "risk": "read", "op": "gateway_export", "inputs": [],
     "description": "Save the gateway radio's own owner, configuration and channels, keys included, to this box at mode 0600, so a replacement radio can be given the same mesh. It is the one radio the Bench cannot reach, and without this copy the key the whole mesh uses exists nowhere but the radio. Nothing is written to any radio. It does not carry the radio's own identity: a replacement is a different radio to the fleet and cannot manage a device until it has been given an admin key."},
    {"id": "gateway_set", "title": "Choose the gateway radio", "risk": "change", "op": "gateway_set",
     "inputs": [{"name": "path", "type": "text", "required": True, "max_bytes": 200, "description": "the radio's /dev/serial/by-id/ path, from gateway's candidates; the box refuses one it cannot see"}],
     "confirm": "The bridge restarts onto that radio and the mesh is down for a few seconds. Nothing is written to any radio, and the devices on the mesh are untouched.",
     "description": "Point this box at a different radio and re-attach to it, without an install or an edit to any file. Only a radio the box can see now may be chosen, because the fault this exists to fix is a box pointed at a radio that is not there. It does not carry the mesh across: a different radio is a different identity, and what that means for the fleet's channel and admin key is its own job."},
    ...
    {"id": "peers", "title": "Peers", "risk": "read", "op": "peers", "inputs": [],
     "description": "This site's identity, the sites it is joined to and their state, the pictures it holds from them (Spec 052)."},
    {"id": "peer_invite", "title": "Invite a peer", "risk": "change", "op": "peer_invite", "inputs": [],
     "confirm": "A one-time code, good for ten minutes, that lets one other site join this one.",
     "description": "Mint a one-time invite (code, address and this site's fingerprint, with its QR) for another Mesh Manager to join this site. Ten minutes, one use."},
    {"id": "peer_join", "title": "Join a site", "risk": "change", "op": "peer_join",
     "inputs": [{"name": "invite", "type": "text", "required": True, "max_bytes": 200, "description": "the invite text as the other site's screen shows it: host:port/code/fingerprint"}],
     "confirm": "This site will send its picture (nodes, positions, battery) to the site the invite names, and show that site's picture here.",
     "description": "Join another Mesh Manager by its invite: the link is proven by the invite's fingerprint and kept up from here."},
    {"id": "peer_sharing_set", "title": "Set what a peer shares", "risk": "change", "op": "peer_sharing_set",
     "inputs": [{"name": "site", "type": "text", "required": True, "max_bytes": 64, "description": "the peer's site id"},
                {"name": "cls", "type": "text", "required": True, "max_bytes": 12, "description": "nodes, messages, waypoints or alerts"},
                {"name": "out", "type": "text", "required": False, "max_bytes": 3, "description": "on or off: this class leaves here for that peer"},
                {"name": "in", "type": "text", "required": False, "max_bytes": 3, "description": "on or off: this class from that peer shows here"},
                {"name": "channels", "type": "text", "required": False, "max_bytes": 40, "description": "messages only: the local channel indexes that leave, comma-separated; empty is every channel"},
                {"name": "air", "type": "text", "required": False, "max_bytes": 3, "description": "on or off: what arrives from that peer goes onto this mesh (messages, waypoints); a radio site only"},
                {"name": "air_channel", "type": "text", "required": False, "max_bytes": 2, "description": "messages only: the local channel index aired messages go out on; empty keeps the arriving index"}],
     "confirm": "The sharing table for that peer changes. With the air on, what arrives from that peer is transmitted on this mesh and costs airtime here.",
     "description": "One class of one peer's sharing table (ADR 003): what leaves here (out) and what is accepted from there (in). Direct messages, keys and admin traffic are not classes and never cross."},
    {"id": "peer_send_text", "title": "Send to a peer's mesh", "risk": "air", "op": "peer_send_text",
     "inputs": [{"name": "site", "type": "text", "required": True, "max_bytes": 64, "description": "the peer's site id"},
                {"name": "channel", "type": "int", "required": False, "min": 0, "max": 7, "description": "the channel at the far site (default 0)"},
                {"name": "text", "type": "text", "required": True, "max_bytes": 180, "description": "the message, 180 bytes at most; the far site adds its prefix"}],
     "confirm": "This goes to that site over the link and onto its mesh only if that site allows it; the receipt says which.",
     "description": "Send a message into a peer's chat over the link (Spec 054). It is transmitted on the far mesh only where that site's Air switch allows, prefixed with this site's name; the receipt reads on the air at <site> or not aired."},
    {"id": "peer_forget", "title": "Forget a peer", "risk": "change", "op": "peer_forget",
     "inputs": [{"name": "site", "type": "text", "required": True, "max_bytes": 64, "description": "the peer's site id"}],
     "confirm": "The pin is dropped: that site can no longer connect, and its picture leaves this screen.",
     "description": "Drop a peer: its pin, its link and its picture."},
    ...
    {"id": "alert_ack", "title": "Acknowledge an alert", "risk": "change", "op": "alert_ack",
     "inputs": [{"name": "node", "type": "text", "required": False, "max_bytes": 12, "description": "the radio id the alert is about"},
                {"name": "kind", "type": "text", "required": False, "max_bytes": 24, "description": "which alert on that node: silent, battery, unknown, fence, key"},
                {"name": "every", "type": "text", "required": False, "max_bytes": 4, "description": "yes to acknowledge every open alert at once"}],
     "confirm": "Acknowledge every open alert. Each stays in the history and none is raised again while its condition holds; if a condition clears and happens again, it alerts afresh.",
     "description": "Take an alert off the open list because someone has seen it. It is remembered rather than deleted: while the condition holds it is not raised again, and when the condition clears the acknowledgement is forgotten so a recurrence alerts afresh. The history keeps it either way."},
