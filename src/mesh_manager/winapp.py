...
def main(argv=None):
    ...
    UP.sweep_kept(UP.app_bundle())   # Spec 065: this run proves the last update worked; let the old copy go
    ...
    watch = UP.Watcher(dirs, D.read_config(dirs["config"]) if os.path.exists(dirs["config"]) else {}, on_quit=run.stop)
    watch.start()
    ...
    def take_update(*_):
        """Spec 065: fetch the waiting version, check its hash, swap the folder over, start the new one."""
        rel = UP.newer_release(UP.fetch_releases(), running=__version__)
        ...
        if out.get("applied") and UP.relaunch(out["bundle"]):
            run.stop()
            os._exit(0)
        print("update: " + str(out.get("error") or "the update did not go in"), flush=True)

    def update_title(_=None):
        return f"Update to {watch.found}" if watch.found else "No update waiting"

    menu = pystray.Menu(
        pystray.MenuItem(lambda _: menu_lines(run.status(), run.url, radio, watch.found)[0], None, enabled=False),
        pystray.MenuItem(lambda _: menu_lines(run.status(), run.url, radio, watch.found)[1], None, enabled=False),
        pystray.Menu.SEPARATOR,
        pystray.MenuItem(update_title, take_update, enabled=lambda _: bool(watch.found)),
        pystray.Menu.SEPARATOR,
